...
export function ProductGrid({ 
  ...
  const { products, error, loading, refetch } = useProductInfo(params);
...
export function ProductFilters({ params, onParamsChange, categories = [] }: ProductFiltersProps) {
  ...
  const handlePriceRangeChange = (startprice?: string, endprice?: string) => {
    onParamsChange({
      ...params,
      startprice,
      endprice,
    ...
    <div className="bg-primary border border-border/10 rounded-lg p-6 mb-6">
      ...
      <form onSubmit={handleSearchSubmit} className="mb-4">
        <div className="flex gap-2">
          ...
          <button
            type="submit"
            className="bg-secondary hover:bg-secondary/80 text-accent px-4 py-2 rounded transition-colors"
          >
            Search
          </button>
        </div>
